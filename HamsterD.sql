...
CREATE TABLE TB_IN_COMMENT(-- 게시물 대댓글 테이블
    ...
    POST_NO NUMBER, -- 외래키, 게시글 SEQ
    CONSTRAINT TB_IN_COMMENT_FK FOREIGN KEY (COMMENT_NO) REFERENCES TB_COMMENT(COMMENT_NO) ON DELETE CASCADE
    -- 대댓글 포함된 댓글 삭제 시 부모키 위배되기 때문에 지정해줘야함
    -- 대댓글은 null 표시되고 db값에는 남아있음
    -- 대댓글도 삭제하고 싶으면  ON DELETE SET NULL로 변경해주면 됨
 );
...
ALTER TABLE TB_IN_COMMENT ADD CONSTRAINT TB_IN_COMMENT_COMMENT_NO_FK FOREIGN KEY(COMMENT_NO) REFERENCES TB_COMMENT; -- 대댓글(TB_IN_COMMENT) 테이블의 TB_COMMENT 컬럼에게 외래키 제약조건 추가(부여) (참조하는 컬럼은 TB_COMMENT 테이블의 기본키인 COMMENT_NO 컬럼이다.)
ALTER TABLE TB_GROUP_COMMENT ADD CONSTRAINT TB_COMMENT_MEMBER_NO_FK FOREIGN KEY(MEMBER_NO) REFERENCES TB_MEMBER; -- 그룹 댓글 : 멤버 SEQ 외래키
ALTER TABLE TB_GROUP_COMMENT ADD CONSTRAINT TB_COMMENT_GROUP_NO_FK FOREIGN KEY(GROUP_NO) REFERENCES TB_STUDYGROUP; -- 그룹 댓글 : 스터디그룹 SEQ 외래키
...
select * from tb_studygroup;
select * from tb_group_rev;
select * from tb_group_comment;
commit;